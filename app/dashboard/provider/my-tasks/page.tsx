...
export default function MyTasksPage() {
  ...
  useEffect(() => {
    if (!token) return;
    axios
    .get<Task[]>("http://localhost:8000/api/tasks/my", {
  ...
  const handleComplete = async (id: number) => {
    try {
      await axios.put(
        `http://localhost:8000/api/tasks/complete/${id}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
            ...
            {tasks.map((task) => (
              ...
              >
                ...
                <button
                  ...
                  className={`mt-4 px-4 py-2 rounded-lg text-white font-semibold transition-all ${
